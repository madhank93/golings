...
func ExitStatus(name string, args ...string) (int, error) {
	...
	// FIXME: separate the two failures. A child that ran and exited non-zero
	// gives an *exec.ExitError carrying its code — report the code and no
	// error. Anything else means the command never started: report -1 and the
	// error. errors.As is how you ask, because err may be wrapped.
	_ = errors.As
	return -1, err
}
