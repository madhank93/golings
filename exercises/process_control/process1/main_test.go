...
func RunAndCapture(name string, args ...string) (string, error) {
	...
	// FIXME: run cmd and return what it printed on stdout, trailing newline
	// trimmed. One method on *exec.Cmd runs it and collects stdout for you —
	// its sibling collects stdout and stderr together, which is not what this
	// function promises.
	_ = cmd
	return "", nil
}
