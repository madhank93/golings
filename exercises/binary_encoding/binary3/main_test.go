...
func ReadFrame(r io.Reader) ([]byte, error) {
	// FIXME: read the four-byte length, then exactly that many bytes of
	// payload. Check the length against MaxFrame *before* allocating — it came
	// off the wire, so it is not yours to trust — and wrap ErrFrameTooLarge
	// when it is out of bounds.
	_ = io.ReadFull
	return nil, fmt.Errorf("ReadFrame is not implemented")
}
