...
func ServeConcurrent(ln net.Listener, c *Counter) error {
	...
	for {
		...
		// FIXME: this handles connections one at a time, so a slow client
		// blocks everyone behind it. Move the body into its own goroutine —
		// and keep the WaitGroup honest, so Serve does not return while a
		...
		func() {
			...
			// A slow client should hold only its own goroutine.
			if line == "SLOW\n" {
