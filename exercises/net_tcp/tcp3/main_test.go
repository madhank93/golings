...
func Serve(ln net.Listener) error {
	...
	conn.Close()
	// FIXME: serve connections until the listener is closed, not just one.
	// Wrap the accept-answer-close above in a loop, and decide what to do with
	// the error Accept returns once ln is closed: errors.Is(err, net.ErrClosed)
	// is the shutdown, not a failure.
	return errors.New("Serve handled one connection and gave up")
}
