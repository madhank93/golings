	...
	for _, ti := range tiers {
		for _, topic := range ti.topics {
			for _, e := range byTopic[topic] {
				...
				fmt.Fprintf(&b, "  { topic: %s, slug: %s, mode: %s, description: %s, path: %s%s%s },\n",
					js(topic), js(e.Name), js(e.Mode), js(e.Description()), js(e.Path), playFields(link, linked), raceField(e))
...
// raceField marks an entry the race detector is load-bearing for. The
// playground has no detector, so the catalog says so rather than letting a
// green run in the browser read as a solved exercise.
func raceField(e exercises.Exercise) string {
	if !e.Race {
		return ""
	}
	return ", race: true"
